...
def save_depth(args):
    ...
    image_loader = DataLoader(
        dataset=dataset, batch_size=args.batch_size, shuffle=False, num_workers=0, drop_last=False)
...
def filter_depth(args, scan: str = ""):
    ...
    for ref_view, src_views in pair_data: 
        ...
        ref_intrinsics, ref_extrinsics = read_cam_file(
            os.path.join(args.input_folder, scan, "cams_1/{:0>8}_cam.txt".format(ref_view)))[0:2]
...
##########################################################
        ...
        for src_view in src_views:
            ...
            src_intrinsics, src_extrinsics = read_cam_file(
                os.path.join(args.input_folder, scan, "cams_1/{:0>8}_cam.txt".format(src_view)))[0:2]
